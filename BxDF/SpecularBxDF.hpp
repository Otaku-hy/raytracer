...
public:
    ...
    Vector3f sample_fr(const Vector3f &w0, Vector3f &wi, float &pdf) override;
...
Vector3f SpecularReflection::sample_fr(const Vector3f &w0, Vector3f &wi, float &pdf)
{
    if (cosTheta(w0) == 0)
    ...
    return Vector3f(fr) / abs(cosTheta(wi));
}
...
Vector3f SpecularTransmission::sample_fr(const Vector3f &w0, Vector3f &wi, float &pdf)
{
    ...
    Vector3f wi = wt_vert + cosThetaT * Vector3f(0, 1, 0);
    pdf = 1.0;

    return Vector3f((1 - fresnel->Evaluate(wi)) * etaItmp * etaTtmp / (etaTtmp * etaTtmp) / abs(cosTheta(wi)));
}
